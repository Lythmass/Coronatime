    ...
    <div class = "px-4 lg:px-28">
        ...
        <form class = "lg:w-[24.5rem]" action="{{route('create-user', [app()->getLocale()])}}" method="post">
            ...
            <div class = "flex flex-col gap-4 lg:gap-6 items-center w-full m-auto">
                <x-session-input title="{{__('login.username')}}" field="username" type="text" placeholder="{{__('register.enter-username')}}"/>
                <x-session-input title="{{__('login.password')}}" field="password" type="password" placeholder="{{__('register.enter-password')}}"/>
            </div>
