<x-sessions-layout class="overflow-x-hidden pb-12">
    <header class="my-6">
        <div class="flex items-center justify-between flex-wrap gap-10 px-4 lg:hidden relative">
            ...
            <label id="img" for="hamburger" class="cursor-pointer">
                <img src="/images/hamburger.png" class="w-6" />
            </label>
            <div id="menu" class="w-full h-full bg-white absolute left-0 top-0">
                <form method="post" action="{{ route('first-login', [app()->getLocale()]) }}"
    ...
    @if (request()['panel'] == 'worldwide')
        <section class="flex px-4 lg:px-28 w-full flex-wrap lg:flex-nowrap gap-4">
            <div
                class="gap-4 w-full lg:w-[40%] lg:text-xl bg-lightBlue h-52 lg:h-64 rounded-2xl flex flex-col justify-center items-center">
            ...
            <div
                class="gap-4 grow w-[40%] lg:text-xl bg-lightGreen h-52 lg:h-64 rounded-2xl flex flex-col justify-center items-center">
            ...
            <div
                class="gap-4 grow w-[40%] lg:text-xl bg-lightYellow h-52 lg:h-64 rounded-2xl flex flex-col justify-center items-center">
    ...
    @else
        <section class="w-full lg:px-28">
            <div class="flex flex-col items-center w-full">
                <form id="search-form" action="{{ route('dashboard', [app()->getLocale(), 'bycountry']) }}"
                    class="w-full flex justify-left items-center gap-3 px-2 lg:px-0">
                    <input
                        ...
                        type="text" name="search" id="search" placeholder="{{ __('dashboard.search') }}"
                ...
                <div class="lg:rounded-t-lg text-sm font-semibold flex w-full bg-neutral-100 py-5 mt-6 pl-4">
                    <x-sort-state
                        href="{{ route('dashboard', [app()->getLocale(), 'bycountry']) }}/?{!! http_build_query(['search' => request()->input('search')]) . '&' !!}{{ request()->input('ascend') ? 'descend' : 'ascend' }}=location"
                        text="{{ __('dashboard.location') }}"
                        state="{{ request('ascend') == 'location' || request('descend') == 'location' ? (request()->keys()[0] == 'search' ? request()->keys()[1] : request()->keys()[0]) : 'non' }}" />
                    <x-sort-state
                        href="{{ route('dashboard', [app()->getLocale(), 'bycountry']) }}/?{!! http_build_query(['search' => request()->input('search')]) . '&' !!}{{ request()->input('ascend') ? 'descend' : 'ascend' }}=confirmed"
                        text="{{ __('dashboard.newcases') }}"
                        state="{{ request('ascend') == 'confirmed' || request('descend') == 'confirmed' ? (request()->keys()[0] == 'search' ? request()->keys()[1] : request()->keys()[0]) : 'non' }}" />
                    <x-sort-state
                        href="{{ route('dashboard', [app()->getLocale(), 'bycountry']) }}/?{!! http_build_query(['search' => request()->input('search')]) . '&' !!}{{ request()->input('ascend') ? 'descend' : 'ascend' }}=death"
                        text="{{ __('dashboard.death') }}"
                        state="{{ request('ascend') == 'death' || request('descend') == 'death' ? (request()->keys()[0] == 'search' ? request()->keys()[1] : request()->keys()[0]) : 'non' }}" />
                    <x-sort-state
                        href="{{ route('dashboard', [app()->getLocale(), 'bycountry']) }}/?{!! http_build_query(['search' => request()->input('search')]) . '&' !!}{{ request()->input('ascend') ? 'descend' : 'ascend' }}=recovered"
                        text="{{ __('dashboard.recovered') }}"
                        state="{{ request('ascend') == 'recovered' || request('descend') == 'recovered' ? (request()->keys()[0] == 'search' ? request()->keys()[1] : request()->keys()[0]) : 'non' }}" />
                </div>
                <div
                    class="lg:rounded-b-lg lg:border lg:border-neutral-100 flex flex-col items-center w-full lg:h-[30rem] lg:overflow-y-auto">
                    ...
                    @foreach ($statistics as $statistic)
                        <div class="flex text-sm w-full justify-between pl-4">
                            <p class="basis-1/4">
                                {{ app()->getLocale() == 'en' ? ucwords($statistic->getCountry->en) : ucwords($statistic->getCountry->ka) }}
                            </p>
